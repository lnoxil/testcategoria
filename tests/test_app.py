...
class QuizAppTests(unittest.TestCase):
    ...
    def test_parse_questions_from_text(self):
        ...
        self.assertEqual(parsed[0]["category"], 5)

    def test_parse_questions_from_text_with_custom_category(self):
        raw = (
            "Вопрос 1\n"
            "Категория 4 вопрос?\n"
            "Выберите один ответ:\n"
        ...
        parsed = app.parse_questions_from_text(raw, category=4, section="Категория 4")
        self.assertEqual(parsed[0]["category"], 4)
        self.assertEqual(parsed[0]["section"], "Категория 4")
        self.assertEqual(parsed[0]["question_number"], 4001)
        self.assertEqual(parsed[0]["original_question_number"], 1)
